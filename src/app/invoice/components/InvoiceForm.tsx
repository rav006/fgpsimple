...
const generateNewInvoiceNumber = (currentInvoiceNumber: string): string => {
  const date = new Date();
  const yyyymmdd =
  ...
  if (currentInvoiceNumber && currentInvoiceNumber.startsWith(prefix)) {
    const sequenceStr = currentInvoiceNumber.substring(prefix.length);
    const sequence = parseInt(sequenceStr, 10);
    const newSequence = (sequence + 1).toString().padStart(4, '0');
...
const InvoiceForm = () => {
  ...
  const addNextInvoiceHandler = () => {
    setInvoiceNumber((prevNumber) => generateNewInvoiceNumber(prevNumber));
    setItems([
      {
        id: uid(6),
        name: '',
  ...
  const edtiItemHandler = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newItems = items.map((item) => {
      if (item.id === id) {
        return { ...item, [name]: name === 'qty' ? Number(value) : value };
      }
      return item;
    });
    setItems(newItems);
